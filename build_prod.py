...
def thread_build():
    if sys.platform == 'win32':
        some_command = 'cmd /k "flutter build apk --flavor production -t lib/main_prod.dart --release"'
        p = subprocess.Popen(some_command)
        p.wait()
    else:
        os.system('/Users/nguyennguyen/flutter/bin/flutter build apk --flavor production -t lib/main_prod.dart --release')
    ...
    while True:
        ...
        if os.path.isfile(path) and timeCreated != os.path.getmtime(path):
            ...
            while not isDone and retryCount < 5:
                try:
                    telegram_notify.send_document(-885205231, document) # Hearlink work group
                    isDone = True
                except:
            ...
            if not isDone:
                ...
                if sys.platform == 'win32':
                    os.system('explorer ' + path.replace('\\app-production-release.apk', ''))
                else:
                    os.system('open ' + path.replace('/app-production-release.apk', ''))
