...
class TypingWidget extends StatelessWidget {
  ...
  String _multiUserTextBuilder(List<CustomerDto> author) {
    if (author.isEmpty) {
      return '';
    } else if (author.length == 1) {
      return '${author.first.fullname} is typing';
    } else if (author.length == 2) {
      return '${author.first.fullname} and ${author[1].fullname}';
    } else {
      return '${author.first.fullname} and ${author.length - 1} others';
    }
  }

  /// Used to specify width of stacking avatars based on number of authors.
  double _getStackingWidth(List<CustomerDto> author, double indicatorWidth) {
    if (author.length == 1) {
...
class TypingAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasImage = author.profiles?.avatars?.first != null;
    ...
    return CircleAvatar(
      backgroundColor: hasImage
          ...
          : color,
      backgroundImage: hasImage ? NetworkImage(author.getAvatarUrl) : null,
      radius: 13,
      child: !hasImage
          ? Text(
              initials,
...
class TypingIndicatorOptions {
  ...
  /// By default its empty list which hides the indicator, see [User].
  final List<CustomerDto> typingUsers;
}
