...
import '../src/domain/dtos/chat/chat_message_dto/chat_message_dto.dart';
...
const chatNameColors = [
  const Color.fromRGBO(74, 136, 217, 1.0),
  // Color(0xffff6767),
  Color(0xff66e0da),
  Color(0xfff5a2d9),
  Color(0xfff0c722),
...
Color getUserAvatarNameColor(String id, List<Color> colors) =>
    colors[id.hashCode % colors.length];

/// Returns user initials (can have only first letter of firstName/lastName or both).
String getUserInitials(CustomerDto user) {
  final splitName = user.fullname.split(" ").map((e) => e.trim()).toList();
...
String getUserName(CustomerDto user) => (user.fullname ?? '').trim();
...
String getVerboseDateTimeRepresentation(
  ...
  final localDateTime = dateTime.add(now.timeZoneOffset);

  final formattedDate = dateFormat != null
      ? dateFormat.format(localDateTime)
      : DateFormat.MMMd(dateLocale).format(localDateTime);
  final formattedTime = timeFormat != null
      ? timeFormat.format(localDateTime)
      : DateFormat.Hm(dateLocale).format(localDateTime);

  if (localDateTime.day == now.day &&
      localDateTime.month == now.month &&
      localDateTime.year == now.year) {
    return '${S.current.txt_today}, $formattedTime';
  }
...
List<Object> calculateChatMessages(
  ...
  final currentUserId = PrefAssist.getMyCustomer().id;

  final now = DateTime.now();
  final dateFormater = DateFormat('yyyy-MM-dd');
  final currentDateString = dateFormater.format(now);

  for (var i = messages.length - 1; i >= 0; i--) {
    final isFirst = i == messages.length - 1;
    final isLast = i == 0;
    final message = messages[i];
    final messageHasCreatedAt = message.insert.when != null;
    ...
    final lastMessageSameAuthor = message.senderId == previousMessage?.senderId;
    final lastTime = message.insert.date.millisecondsSinceEpoch -
        (previousMessage?.insert.date.millisecondsSinceEpoch ?? 0);
    final nextTime = (nextMessage?.insert.date.millisecondsSinceEpoch ?? 0) -
        message.insert.date.millisecondsSinceEpoch;
    final isFirstInGroup =
        (!lastMessageSameAuthor || lastTime > groupMessagesThreshold);

    final isLastInGroup =
        (!nextMessageSameAuthor || nextTime > groupMessagesThreshold);
    final onlyMessageInGroup = isFirstInGroup && isLastInGroup;
    final showAvatar = message.senderId != nextMessage?.senderId &&
        message.senderId != currentUserId;

    if (message.insert.date != null && previousMessage?.insert.date != null) {
      final messageDateString =
          dateFormater.format(message.insert.date.toLocal());
      if (messageDateString != currentDateString &&
          message.insert.date.day != previousMessage!.insert.date.day) {
        chatMessages.insert(
          0,
          DateHeader(
            dateTime: message.insert.date.toLocal(),
            text: customDateHeaderText != null
                ? customDateHeaderText(
                    message.insert.date.toLocal(),
                  )
      ...
      } else if (isFirst && messageHasCreatedAt ||
          lastTime > groupMessagesThreshold) {
        chatMessages.insert(
          0,
          DateHeader(
            dateTime: message.insert.date.toLocal(),
    ...
    chatMessages.insert(0, {
      'message': message,
      'firstMessageInGroup': isFirstInGroup,
      'showAvatar': showAvatar,
      'showName': false,
      'showStatus': isLastInGroup,
...
bool isEmoji(String text) {
  ...
  return multiEmojiRegExp.hasMatch(text);
}
