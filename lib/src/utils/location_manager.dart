...
class LocationManager {
  ...
  static LocationManager shared() => _shared;
  final _host = 'http://ip-api.com/json';
  CountryPhone? savedCountry;

  Future<LocationDto?> getCurrentLocation() async {
    if (await Permission.location.isGranted) {
      final position = await Geolocator.getCurrentPosition(
          desiredAccuracy: LocationAccuracy.high);
      return LocationDto(lat: position.latitude.toString(), long: position.longitude.toString());
    } else {
      return PrefAssist.getMyCustomer().location;
    }
  }

  Future<LocationDto?> requestLocation() async {
  ...
  Future<CountryPhone?> getCountryPhone() async {
    ...
    if (response.statusCode == 200) {
      ...
      final code =
          WidgetsBinding.instance.platformDispatcher.locale.countryCode ?? 'VN';
      return _countryFromCode(code);
    }
  }

  CountryPhone? _countryFromCode(String code) {
    final country = Countries.allCountries
...
class CountryPhone {
  CountryPhone(
      {required this.phoneCode, required this.countryCode, required this.name});
